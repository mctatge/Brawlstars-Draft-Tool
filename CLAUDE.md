...
- Frontend `npm run dev` hits the **deployed** API by default — see [docs/dev-commands.md](docs/dev-commands.md).
...
| [docs/spatial-world-model.md](docs/spatial-world-model.md) | Static mechanics + semantic-map world model: scope, schema, resource bounds, validation, and rollout gate | Before changing `backend/bsdraft/world/`, its geometry bootstrap/artifact, or adding a mechanics signal to scoring |
