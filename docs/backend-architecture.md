...
- `engine/` — the draft brain. `engine/state.py`'s `DraftState` is the object threaded
  ...
  with deterministic per-brawler opening jobs from `positioning.py` (curated exact geometry for
  profiled maps, honest mode-level fallbacks elsewhere), plus a data-backed read (head-to-head
  grid, per-map form, ally pairs, the model's win prob for the finished draft) that takes the
  bracket `DraftStats` and the model from `DraftEngine`. Both halves degrade independently — no
  stats/model yields exactly the rule-based plan, and each data section drops out when its cells
  fall under its sample floor.
