...
def test_owned_boosted_brawler_is_priced_as_fully_maxed():
    ...
    try:
        ...
        roster = [S.OwnedBrawler(
            id=free, mastery=0.42, power=9, gaps=[GAP_NO_STAR_POWER],
        ...
        assert m.fielded().buffies == M.Fielded.ready().buffies
...
def test_buffie_ownership_survives_the_request_roster_bridge():
    flags = {"gadget": False, "star_power": True, "hypercharge": False}
    roster = [S.OwnedBrawler(id=A11, mastery=0.5, power=11, buffies=flags)]
    req = S.RecommendRequest(map_id=1, mode="Brawl Ball", rank_bracket="Legendary",
                             personalize=True, roster=roster)
    assert M._roster_for(req)[A11].fielded().buffies == flags
